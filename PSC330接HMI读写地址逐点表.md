...
| 1229 | AiCfgEnable(0) | RW | 0/1 | - | AI0 房间压差 Enable：0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| 1230 | AiCfgType(0) | RW | 0/1 | - | AI0 房间压差 Type：0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| 1231 | AiCfgSigLow(0) | RW | int16 | - | AI0 房间压差 SigLow：信号下限（传感器下限对应的原始采样值） |
| 1232 | AiCfgSigHigh(0) | RW | int16 | - | AI0 房间压差 SigHigh：信号上限（传感器上限对应的原始采样值） |
| 1233 | AiCfgEngLow(0) | RW | int16 | - | AI0 房间压差 EngLow：工程量下限（主） |
| 1234 | AiCfgEngHigh(0) | RW | int16 | - | AI0 房间压差 EngHigh：工程量上限（主） |
| 1235 | AiCfgEngLow2(0) | RW | int16 | - | AI0 房间压差 EngLow2：副工程量下限；仅 AI3 风量通道使用，其余填0 |
| 1236 | AiCfgEngHigh2(0) | RW | int16 | - | AI0 房间压差 EngHigh2：副工程量上限；仅 AI3 风量通道使用，其余填0 |
| 1237 | AiCfgOffset(0) | RW | int16 | - | AI0 房间压差 Offset：校准偏移，叠加在线性换算结果上 |
| 1238 | AiCfgFaultLow(0) | RW | 0.. | - | AI0 房间压差 FaultLow：断线判据：原始值低于它判失效；0=不判 |
| 1239 | AiCfgUnit(0) | RW | 0..5 | - | AI0 房间压差 Unit：0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| 1240 | AiCfgDecimals(0) | RW | 0..3 | - | AI0 房间压差 Decimals：显示小数位（仅显示） |
| 1241 | AiCfgEnable(1) | RW | 0/1 | - | AI1 房间湿度 Enable：0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| 1242 | AiCfgType(1) | RW | 0/1 | - | AI1 房间湿度 Type：0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| 1243 | AiCfgSigLow(1) | RW | int16 | - | AI1 房间湿度 SigLow：信号下限（传感器下限对应的原始采样值） |
| 1244 | AiCfgSigHigh(1) | RW | int16 | - | AI1 房间湿度 SigHigh：信号上限（传感器上限对应的原始采样值） |
| 1245 | AiCfgEngLow(1) | RW | int16 | - | AI1 房间湿度 EngLow：工程量下限（主） |
| 1246 | AiCfgEngHigh(1) | RW | int16 | - | AI1 房间湿度 EngHigh：工程量上限（主） |
| 1247 | AiCfgEngLow2(1) | RW | int16 | - | AI1 房间湿度 EngLow2：副工程量下限；仅 AI3 风量通道使用，其余填0 |
| 1248 | AiCfgEngHigh2(1) | RW | int16 | - | AI1 房间湿度 EngHigh2：副工程量上限；仅 AI3 风量通道使用，其余填0 |
| 1249 | AiCfgOffset(1) | RW | int16 | - | AI1 房间湿度 Offset：校准偏移，叠加在线性换算结果上 |
| 1250 | AiCfgFaultLow(1) | RW | 0.. | - | AI1 房间湿度 FaultLow：断线判据：原始值低于它判失效；0=不判 |
| 1251 | AiCfgUnit(1) | RW | 0..5 | - | AI1 房间湿度 Unit：0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| 1252 | AiCfgDecimals(1) | RW | 0..3 | - | AI1 房间湿度 Decimals：显示小数位（仅显示） |
| 1253 | AiCfgEnable(2) | RW | 0/1 | - | AI2 CO2 Enable：0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| 1254 | AiCfgType(2) | RW | 0/1 | - | AI2 CO2 Type：0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| 1255 | AiCfgSigLow(2) | RW | int16 | - | AI2 CO2 SigLow：信号下限（传感器下限对应的原始采样值） |
| 1256 | AiCfgSigHigh(2) | RW | int16 | - | AI2 CO2 SigHigh：信号上限（传感器上限对应的原始采样值） |
| 1257 | AiCfgEngLow(2) | RW | int16 | - | AI2 CO2 EngLow：工程量下限（主） |
| 1258 | AiCfgEngHigh(2) | RW | int16 | - | AI2 CO2 EngHigh：工程量上限（主） |
| 1259 | AiCfgEngLow2(2) | RW | int16 | - | AI2 CO2 EngLow2：副工程量下限；仅 AI3 风量通道使用，其余填0 |
| 1260 | AiCfgEngHigh2(2) | RW | int16 | - | AI2 CO2 EngHigh2：副工程量上限；仅 AI3 风量通道使用，其余填0 |
| 1261 | AiCfgOffset(2) | RW | int16 | - | AI2 CO2 Offset：校准偏移，叠加在线性换算结果上 |
| 1262 | AiCfgFaultLow(2) | RW | 0.. | - | AI2 CO2 FaultLow：断线判据：原始值低于它判失效；0=不判 |
| 1263 | AiCfgUnit(2) | RW | 0..5 | - | AI2 CO2 Unit：0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| 1264 | AiCfgDecimals(2) | RW | 0..3 | - | AI2 CO2 Decimals：显示小数位（仅显示） |
| 1265 | AiCfgEnable(3) | RW | 0/1 | - | AI3 送风压/风量 Enable：0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| 1266 | AiCfgType(3) | RW | 0/1 | - | AI3 送风压/风量 Type：0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| 1267 | AiCfgSigLow(3) | RW | int16 | - | AI3 送风压/风量 SigLow：信号下限（传感器下限对应的原始采样值） |
| 1268 | AiCfgSigHigh(3) | RW | int16 | - | AI3 送风压/风量 SigHigh：信号上限（传感器上限对应的原始采样值） |
| 1269 | AiCfgEngLow(3) | RW | int16 | - | AI3 送风压/风量 EngLow：工程量下限（主） |
| 1270 | AiCfgEngHigh(3) | RW | int16 | - | AI3 送风压/风量 EngHigh：工程量上限（主） |
| 1271 | AiCfgEngLow2(3) | RW | int16 | - | AI3 送风压/风量 EngLow2：副工程量下限；仅 AI3 风量通道使用，其余填0 |
| 1272 | AiCfgEngHigh2(3) | RW | int16 | - | AI3 送风压/风量 EngHigh2：副工程量上限；仅 AI3 风量通道使用，其余填0 |
| 1273 | AiCfgOffset(3) | RW | int16 | - | AI3 送风压/风量 Offset：校准偏移，叠加在线性换算结果上 |
| 1274 | AiCfgFaultLow(3) | RW | 0.. | - | AI3 送风压/风量 FaultLow：断线判据：原始值低于它判失效；0=不判 |
| 1275 | AiCfgUnit(3) | RW | 0..5 | - | AI3 送风压/风量 Unit：0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| 1276 | AiCfgDecimals(3) | RW | 0..3 | - | AI3 送风压/风量 Decimals：显示小数位（仅显示） |
| 1277 | AiCfgEnable(4) | RW | 0/1 | - | AI4 新风湿度 Enable：0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| 1278 | AiCfgType(4) | RW | 0/1 | - | AI4 新风湿度 Type：0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| 1279 | AiCfgSigLow(4) | RW | int16 | - | AI4 新风湿度 SigLow：信号下限（传感器下限对应的原始采样值） |
| 1280 | AiCfgSigHigh(4) | RW | int16 | - | AI4 新风湿度 SigHigh：信号上限（传感器上限对应的原始采样值） |
| 1281 | AiCfgEngLow(4) | RW | int16 | - | AI4 新风湿度 EngLow：工程量下限（主） |
| 1282 | AiCfgEngHigh(4) | RW | int16 | - | AI4 新风湿度 EngHigh：工程量上限（主） |
| 1283 | AiCfgEngLow2(4) | RW | int16 | - | AI4 新风湿度 EngLow2：副工程量下限；仅 AI3 风量通道使用，其余填0 |
| 1284 | AiCfgEngHigh2(4) | RW | int16 | - | AI4 新风湿度 EngHigh2：副工程量上限；仅 AI3 风量通道使用，其余填0 |
| 1285 | AiCfgOffset(4) | RW | int16 | - | AI4 新风湿度 Offset：校准偏移，叠加在线性换算结果上 |
| 1286 | AiCfgFaultLow(4) | RW | 0.. | - | AI4 新风湿度 FaultLow：断线判据：原始值低于它判失效；0=不判 |
| 1287 | AiCfgUnit(4) | RW | 0..5 | - | AI4 新风湿度 Unit：0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| 1288 | AiCfgDecimals(4) | RW | 0..3 | - | AI4 新风湿度 Decimals：显示小数位（仅显示） |
...
## 7. AI 通道配置地址段（2026-09-14 新增）

> 新增「模拟量通道配置」功能：每个通道 12 个字，字段顺序固定。
> 改动经固件换算，**直接影响压力/温湿度等工程量的计算**，进而影响保护与风机控制，请谨慎填写。

### 7.1 每通道 12 字字段定义（偏移 +0~+11）

| 偏移 | 字段 | 单位 | 取值范围 | 说明 |
|---:|---|---|---|---|
| +0 | Enable | - | 0/1 | 0=禁用 1=启用；置0时该通道输出失效标记，不参与控制 |
| +1 | Type | - | 0/1 | 0=0-10V 1=4-20mA；仅决定按哪种传感器填信号上下限 |
| +2 | SigLow | 0.1V/mA | int16 | 信号下限（传感器下限对应的原始采样值） |
| +3 | SigHigh | 0.1V/mA | int16 | 信号上限（传感器上限对应的原始采样值） |
| +4 | EngLow | 工程量 | int16 | 工程量下限（主） |
| +5 | EngHigh | 工程量 | int16 | 工程量上限（主） |
| +6 | EngLow2 | 工程量 | int16 | 副工程量下限；仅 AI3 风量通道使用，其余填0 |
| +7 | EngHigh2 | 工程量 | int16 | 副工程量上限；仅 AI3 风量通道使用，其余填0 |
| +8 | Offset | 工程量 | int16 | 校准偏移，叠加在线性换算结果上 |
| +9 | FaultLow | 0.1V/mA | 0.. | 断线判据：原始值低于它判失效；0=不判 |
| +10 | Unit | - | 0..5 | 0=Pa 1=℃ 2=%RH 3=ppm 4=CMH 5=Bar（仅显示） |
| +11 | Decimals | - | 0..3 | 显示小数位（仅显示） |

### 7.2 330 内机 AI 配置块 —— Parameter[1229..1288]（RW）

5 个通道，每通道 12 字。地址 = 基址 + 通道 × 12。
...
| AI2 CO2 | 1253 | 1253 | 1254 | 1255 | 1256 | 1257 | 1258 | 1259 | 1260 | 1261 | 1262 | 1263 | 1264 |
...
**持久化**：由固件 `IndoorAiJournal` 原子双槽日志保存（EEPROM 1152 / 1280），
断电瞬间写入不会写坏；首次上电自动从旧参数播种。

### 7.3 316 外机 AI 配置（经 330 网关）—— HMI Parameter[2680..2875]（RW）

**本工具/本表只连 330**，316 的配置由 330 每秒轮询镜像到这段 HMI 地址，
写入也由 330 转发给对应外机。每台外机 49 字（4 通道 × 12 + 1 版本标记）。

| 外机 | 范围 | AI0 基址 | AI1 基址 | AI2 基址 | AI3 基址 | 版本标记 |
|---|---|---:|---:|---:|---:|---:|
| 1# | 2680..2728 | 2680 | 2692 | 2704 | 2716 | 2728 |
| 2# | 2729..2777 | 2729 | 2741 | 2753 | 2765 | 2777 |
| 3# | 2778..2826 | 2778 | 2790 | 2802 | 2814 | 2826 |
| 4# | 2827..2875 | 2827 | 2839 | 2851 | 2863 | 2875 |

> 版本标记 = **12641（0x3161）** 表示该外机配置已迁移完成；只读，写它会被网关拒绝。
> 系统最多 4 台外机；未配置的外机整段读到 0 属正常。

### 7.4 316 本地地址（一般不需要直接访问）

| 地址 | 说明 |
|---|---|
| 302~349 | 316 本地的 AI 配置块（4 通道 × 12 字），随外机参数区保存 |
| 350 | 版本标记，固件写 12641 表示已迁移 |
| 1528~1576 | 316 向 330 发布的只读镜像（每周期刷新，写它会被覆盖） |

> ⚠️ 316 侧 302~350 落在参数持久区 0~499 内，**上电时会被 `Readeeprom()` 做范围体检**：
> 超出该字段 `Parameter_min/max` 的值会在**保存时**被回退成上一个有效值（约 1 秒内，不会等到重启）。

### 7.5 关键注意事项

| 事项 | 说明 |
|---|---|
| **单位差异** | 330 的信号上下限是 **0.1V/0.1mA**（50 = 5.0V）；316 是**原始采样端点**（400 ≈ 0.5V，2000 ≈ 4.5V） |
| **316 压力工程量** | 单位 **0.01 Bar**（5000 = 50.00 Bar） |
| **生效范围** | 配置直接改变压力/温湿度换算结果 → 影响高压/低压保护、冷凝风机、化霜、EEV 判据 |
| **网关限制** | 316 的配置写入**只能逐个寄存器写（06 功能码）**；用 16 功能码多寄存器写会被 330 拒绝 |
| **建议时机** | 316 的改动在**机组停机**时进行 |
